...
class VAE(nn.Module):
    ...
    def rhythm_decoder2(self, z):
        out = torch.zeros((z.size(0), self.rhythm_dims))
        out[:, -1] = 1.
        x = []
        hx = torch.tanh(self.linear_init_0(z))
        if torch.cuda.is_available():
            out = out.cuda()
        for i in range(self.n_step):
            out1 = torch.cat([out, z], 1)
            hx2 = self.grucell_0(out1, hx)
            hx = hx2
            out3 = F.log_softmax(self.linear_out_0(hx2), 1)
            x.append(out3)
            if self.training:
                p = torch.rand(1).item()
                if p < self.eps:
                    out4 = self.rhythm_sample[:, i, :]
                else:
                    out4 = self._sampling(out3)
            else:
                out4 = self._sampling(out3)
            out = out4
        return torch.stack(x, 1)
